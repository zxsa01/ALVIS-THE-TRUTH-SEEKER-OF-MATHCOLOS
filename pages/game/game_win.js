...
export default class Game_Win extends Component {
    render () {

        const { id_subject, name_subject, id_player, id_education_level, score_input } = this.props.route.params;

        return (
            <ImageBackground source = { require("../../images/background_game.jpg") } resizeMode = "cover" style = { styles.imagebackground }>
                <View style = { styles.container }>
                    <Text style = { styles.text }>GAME WIN</Text>
                    <Text style = { styles.text }>SCORE: { score_input }</Text>
                    <Button_Cycle
                        title = "MAIN MENU"
